...
/* mod end*/
  ...
  /* mod for OSafe Date Field */
  Date.prototype.toFormattedString_osafe_default = function(include_time){
      var hour, str;
      var fourDigitYear = this.getFullYear();
      var twoDigitYear = new String(fourDigitYear).substr(2,2);
      str = Date.padded2(this.getMonth() + 1) + "/" + Date.padded2(this.getDate()) + "/" +twoDigitYear;

      if (include_time) { hour=this.getHours(); str += " " + this.getHours() + ":" + this.getPaddedMinutes() + ":" + this.getSeconds() + "." + this.getMilliseconds() }
      return str;
  }
//Date.parseFormattedString = function(string) { return new Date(string);}
  //from format_american.js
  Date.parseFormattedString = function (string) {
      ...
      // 1/1/1111 01:11pm
      // 1/1/1111 1:11pm
      var regexp = "(([0-1]?[0-9])\/[0-3]?[0-9]\/[0-9]{2}) *([0-9]{1,2}(:[0-9]{2})? *(am|pm))?";
...
CalendarDateSelect.prototype = {
  ...
  dateString: function() {
      ...
      return (this.selection_made) ? this.selected_date.toFormattedString_osafe_default(this.use_time) : "&#160;";
  },
  parseDate: function()
  {
